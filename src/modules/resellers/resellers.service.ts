...
export class ResellersService {
  ...
  /**
   * 2️⃣ Search (DB-level, indexed JSONB access)
   */
  if (search) {
    const s = `%${search}%`;
    supabaseQuery = supabaseQuery.or(
  ...
  /**
   * 3️⃣ Pagination ONLY if requested
   */
  let safePage = 1;
  let safeLimit = 0;
  ...
  const resellers = (data ?? []).map(row => row.value);
  const totalItems = count ?? resellers.length;

  /**
   * 5️⃣ Response shape (pagination optional)
   */
  if (!isPaginationRequested) {
    return {
      resellers,
